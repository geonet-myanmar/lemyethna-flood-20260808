...
DATA AVAILABILITY (checked against the GEE catalogue on 2026-08-08)
  ...
  GEE ingestion lags acquisition by a few days, and the two Sentinel-1 GRD
  collections do not land together: COPERNICUS/S1_GRD_FLOAT (linear power) is
  routinely populated before COPERNICUS/S1_GRD (dB).  The 2026-08-08 scene was
  in _FLOAT alone on the day it was acquired.  Since S1_GRD is exactly
  10*log10(S1_GRD_FLOAT), preflight() picks whichever collection carries BOTH
  dates -- preferring the dB one -- and load_s1() converts if needed.  Both
  composites always come from the same collection; the pair is never split.
  If neither collection has both dates, the script exits with a table of what
  IS available.  Re-run unchanged once the missing scene lands.
